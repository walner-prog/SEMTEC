<div class="max-w-7xl mx-auto py-12 px-6">
    ...
    <div class="text-center mb-10 relative px-4 sm:px-0">
        ...
        <div class="flex flex-col sm:flex-row justify-center gap-4 sm:gap-6 mt-6">
            ...
            <!-- Puntos -->
            <div
                class="bg-yellow-100 dark:bg-yellow-900 p-4 sm:p-6 rounded-2xl shadow-lg transform hover:scale-105 transition-transform flex flex-col items-center animate-bounce">
            ...
            <!-- Monedas -->
            <div
                class="bg-orange-100 dark:bg-orange-900 p-4 sm:p-6 rounded-2xl shadow-lg transform hover:scale-105 transition-transform flex flex-col items-center animate-bounce delay-75">
            ...
            <!-- Trofeos -->
            <div
                class="bg-green-100 dark:bg-green-900 p-4 sm:p-6 rounded-2xl shadow-lg transform hover:scale-105 transition-transform flex flex-col items-center animate-bounce delay-150">
        ...
        <div class="absolute inset-0 pointer-events-none overflow-hidden">
            @for ($i = 0; $i < 15; $i++) <span
                class="absolute w-2 h-2 sm:w-3 sm:h-3 bg-yellow-400 rounded-full top-0 left-[{{ rand(0, 95) }}%] animate-fall-slow">
                </span>
                @endfor
        </div>
    ...
    <section aria-labelledby="games-heading" class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 w-full">
            ...
            <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                <select wire:model.live="categoriaSeleccionada"
                    class="w-full sm:w-auto px-4 py-2 rounded-xl border border-gray-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-gray-700 dark:text-slate-100">
                ...
                <button wire:click="$set('categoriaSeleccionada', '')"
                    class="w-full sm:w-auto px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-semibold transition-all">
                    Limpiar Filtro
                </button>
                @endif
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <article
                class="bg-white dark:bg-slate-800 rounded-3xl p-6 border-4 border-emerald-200 dark:border-emerald-900 hover:border-emerald-500 dark:hover:border-emerald-400 transition-all shadow-lg hover:shadow-2xl">

                <!-- Header con icono y XP -->
                <div class="flex items-start justify-between mb-4">
                    <div
                        class="w-16 h-16 bg-emerald-100 dark:bg-emerald-900 rounded-2xl flex items-center justify-center">
                        <i class="fa-solid {{ $juego->icono }} text-4xl text-emerald-600 dark:text-emerald-400"></i>
                    </div>
                    <span
                        class="px-3 py-1 bg-emerald-100 dark:bg-emerald-900 text-emerald-700 dark:text-emerald-300 rounded-full text-sm font-semibold">
                        Nivel {{ $juego->nivel }}
                    </span>
                </div>
                ...
                <div class="flex flex-col gap-2 mt-4 text-center">
                    ...
                    <div class="flex justify-center gap-2 flex-wrap mt-2">
                        ...
                        {{-- <span
                            class="bg-yellow-100 dark:bg-yellow-800 text-yellow-800 dark:text-yellow-200 px-3 py-1 rounded-full text-xs font-semibold shadow">
                            {{ $logro->titulo }}
                        </span> --}}
                        @endforeach
                    </div>
                ...
                <div class="flex justify-center gap-6 mb-4 text-gray-500 dark:text-gray-300">
                    <button wire:click="reaccionar({{ $juego->id }}, 'me_gusta')"
                        class="flex items-center gap-2 transition {{ $juego->reacciones->where('user_id', auth()->id())->where('tipo','me_gusta')->count() ? 'text-blue-500' : 'hover:text-blue-500' }}">
                        <i class="fa-solid fa-thumbs-up"></i>
                        <span>{{ $juego->me_gusta_count }}</span>
                    </button>

                    <button wire:click="reaccionar({{ $juego->id }}, 'corazon')"
                        class="flex items-center gap-2 transition {{ $juego->reacciones->where('user_id', auth()->id())->where('tipo','corazon')->count() ? 'text-rose-500' : 'hover:text-rose-500' }}">
                ...
                <button wire:click="abrirJuego({{ $juego->id }})"
                    class="w-full py-4 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-bold text-lg transition-all focus-visible-ring">
            ...
            <article
                class="relative bg-gray-100 dark:bg-slate-800 rounded-3xl p-6 border-4 border-gray-300 dark:border-slate-700 shadow-lg flex flex-col items-center justify-center opacity-70">
                ...
                <!-- Overlay bloqueado -->
                <div
                    class="absolute inset-0 bg-black bg-opacity-50 rounded-3xl flex flex-col items-center justify-center text-white backdrop-blur-sm">
    ...
    leerTexto(texto) {
        ...
        <div
            class="bg-gradient-to-b from-blue-200 to-white dark:from-blue-900 dark:to-gray-900 rounded-3xl shadow-2xl relative w-full max-w-3xl p-6 md:p-8 max-h-[90vh] overflow-y-auto">
            <div class="fixed bottom-5 right-5 flex flex-col gap-2 z-50">
                <button
                    @click="document.documentElement.style.fontSize = (parseInt(getComputedStyle(document.documentElement).fontSize)+2)+'px'"
                    class="px-3 py-2 bg-blue-500 text-white rounded-full shadow-md hover:bg-blue-600">A+</button>
                <button
                    @click="document.documentElement.style.fontSize = (parseInt(getComputedStyle(document.documentElement).fontSize)-2)+'px'"
                    class="px-3 py-2 bg-blue-500 text-white rounded-full shadow-md hover:bg-blue-600">A-</button>
                <button @click="document.body.classList.toggle('high-contrast')"
                    class="px-3 py-2 bg-yellow-500 text-black rounded-full shadow-md hover:bg-yellow-600">⚡
                    Contraste</button>
            </div>

            <!-- Botón cerrar -->
            <button wire:click="cerrarModal"
                class="absolute top-5 right-5 text-gray-700 dark:text-gray-200 hover:text-red-500 text-3xl bg-white dark:bg-gray-800 rounded-full w-10 h-10 flex items-center justify-center shadow-md transition-all hover:scale-110 animate-bounce">
                &times;
            </button>
            ...
            <div class="text-center mb-6">
                <h2
                    class="text-4xl md:text-5xl font-extrabold mb-2  text-indigo-800 animate-pulse">
                    {{ $juegoSeleccionado->nombre }} <span class="text-5xl animate-bounce"></span>
                </h2>
                <p class="text-blue-700 dark:text-gray-300 text-lg md:text-xl font-medium">
                ...
                <div class="flex flex-col gap-1 mt-3 text-center text-sm text-gray-700 dark:text-gray-300">
                    ...
                    <div class="flex justify-center gap-2 flex-wrap mt-2">
                        ...
                        <span
                            class="{{ $color['bg'] }} {{ $color['text'] }} px-2 py-0.5 rounded-full text-xs font-semibold shadow flex items-center gap-1">
            ...
            <div class="flex flex-wrap justify-center gap-4 mb-6 items-center">
                ...
                <div
                    class="flex items-center gap-2 bg-yellow-100 dark:bg-yellow-800 px-4 py-2 rounded-full shadow-lg animate-pulse hover:shadow-xl">
                ...
                <div
                    class="flex items-center gap-2 bg-green-100 dark:bg-green-800 px-4 py-2 rounded-full shadow-lg animate-pulse hover:shadow-xl">
                ...
                <div class="flex items-center gap-3">
                    <i
                        class="fa-solid fa-bookmark text-blue-500 text-2xl cursor-pointer hover:scale-110 transition-transform"></i>
                    <i
                        class="fa-solid fa-share-nodes text-green-500 text-2xl cursor-pointer hover:scale-110 transition-transform"></i>
                    <i
                        class="fa-solid fa-trophy text-red-500 text-2xl cursor-pointer hover:scale-110 transition-transform"></i>
                </div>

                <!-- Botones nicas -->
                <button wire:click="generarPista" wire:loading.attr="disabled" wire:target="generarPista"
                    class="flex items-center gap-2 px-5 py-3 bg-blue-500 hover:bg-yellow-400 text-white font-bold rounded-full shadow-md text-xl transform hover:scale-110 transition-all">
                    💡 Pista 🌽
                </button>

                <button wire:click="pedirExplicacion" wire:loading.attr="disabled" wire:target="pedirExplicacion"
                    class="flex items-center gap-2 px-5 py-3 bg-purple-500 hover:bg-indigo-400 text-white font-bold rounded-full shadow-md text-xl transform hover:scale-110 transition-all">
                    📝 Explicación 🏝️
                </button>
            ...
            @if($indicePregunta < count($preguntas)) <div class="mb-4">
                ...
                <!-- Enunciado -->
                <div
                    class="bg-blue-50 dark:bg-blue-900 p-5 rounded-2xl mb-6 shadow-md flex items-center justify-between">
                    <p class="text-center text-3xl flex-1 leading-relaxed
                       bg-gradient-to-r from-indigo-800 font-bold via-blue-900 to-blue-900
                        bg-clip-text text-transparent drop-shadow-[0_2px_6px_rgba(0,0,0,0.25)]
                         animate-pulse" style="font-family: 'Fredoka One', cursive;">
                    ...
                    <button @click="leerTexto(@js($preguntas[$indicePregunta]['enunciado']))"
                        class="ml-4 px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-full shadow-md text-lg transform hover:scale-125 transition-all">
                        🔊 Escuchar
                    </button>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-6">
                    ...
                    <button wire:click="responder('{{ $opcion }}')"
                        class="w-full px-5 py-4 bg-white  text-green-600 dark:text-blue-300 dark:bg-gray-800 border-2 border-blue-400 dark:border-blue-600 rounded-2xl font-bold text-lg shadow-md transition-transform hover:scale-105 hover:bg-blue-50 dark:hover:bg-blue-700 flex items-center justify-center gap-3">
                        <span
                            class="text-xl rounded-full w-10 h-10 flex items-center justify-center bg-gradient-to-tr from-yellow-300 to-blue-300 text-white shadow-lg animate-bounce">
                            {{ chr(65 + $index) }}
                        </span>
                        {{ $opcion }}
                    </button>
                ...
                <div class="my-4 flex items-center justify-center gap-3 text-xl font-bold animate-fade-in">
                    ...
                    <div
                        class="flex items-center gap-3 dark:text-gray-300  bg-green-100 dark:bg-green-800 px-4 py-2 rounded-2xl shadow-lg">
                        <i class="fa-solid fa-circle-check text-green-500 text-3xl"></i>
                        <span class="dark:text-gray-300">¡Bien hecho, campeón! {{ $mensajeMotivador }}</span>
                    </div>
                    @else
                    <div
                        class="flex items-center gap-3 dark:text-gray-300  bg-red-100 dark:bg-red-800 px-4 py-2 rounded-2xl shadow-lg">
                ...
                @if($mensajeFinal)
                <div
                    class="my-4 flex items-center dark:text-gray-300  justify-center gap-3 text-xl font-bold animate-fade-in">
                ...
                <div wire:loading.flex wire:target="generarPista" class="mt-4 justify-center">
                    <div
                        class="flex items-center gap-2 px-4 py-2 bg-yellow-100 dark:bg-yellow-800 rounded-2xl shadow-md">
                ...
                <div wire:loading.flex wire:target="pedirExplicacion" class="mt-4 justify-center">
                    <div
                        class="flex items-center gap-2 px-4 py-2 bg-purple-100 dark:bg-purple-800 rounded-2xl shadow-md">
